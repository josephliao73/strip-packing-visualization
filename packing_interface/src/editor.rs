...
use crate::types::{BottomPanelTab, CodeLanguage, Input, MultipleRunResult};
use iced::highlighter::Theme as HighlighterTheme;
use iced::widget::{button, column, container, mouse_area, pick_list, progress_bar, row, scrollable, text, text_editor};
...
pub fn build_code_panel<'a>(state: EditorState<'a>) -> Element<'a, Input> {
    ...
    content
        .push(column![].height(8))
        .push(code_editor)
        .push(column![].height(12))
        .push(bottom_panel)
        .width(Length::Fill)
        .height(Length::Fill)
        .into()
